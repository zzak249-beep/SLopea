...
async def _process_symbol(
    ...
    # ── FIX v6.3.7: notify_signal solo cuando el trade va a abrirse ──────────
    # Evita flood de Telegram (429) al no enviar señales que serán bloqueadas.
    # En SIGNAL mode: notificar siempre. En LIVE mode: notificar solo si pasa
    # todos los filtros de risk y sizing.
    ...
    # Notificar señal solo cuando el trade va a abrirse — evita 429
    await tg.notify_signal(sig)
...
async def scan_loop(
    ...
    while True:
        ...
        # Doble fallback: si aun así llega 0, usar CAPITAL de config
        if balance <= 0:
            balance = max(C.CAPITAL, 10.0)
            log.warning("Balance sigue en 0 — forzado a CAPITAL=%.2f USDT", balance)
